...
{
    [SerializeField] private float xOffset = 50.0f;
    [SerializeField] private float yOffset = 50.0f;
    [SerializeField] private Transform _worldMapTransform;

    void Awake()
    {
        _worldMapTransform = GameObject.FindGameObjectWithTag("WorldMap").transform;
    }

    public void SpawnEnemy(GameObject enemyPrefab)
    {
        int flag = Random.Range(0, 2);
        Vector3 playerPosition = _worldMapTransform.position;
    ...
    private void OnDrawGizmos()
    {
        if (_worldMapTransform == null)
        {
            ...
            if (player != null) _worldMapTransform = player.transform;
            else return;
        }

        float xOffset = this.xOffset;
        float yOffset = this.yOffset;
        Vector3 center = _worldMapTransform.position;
